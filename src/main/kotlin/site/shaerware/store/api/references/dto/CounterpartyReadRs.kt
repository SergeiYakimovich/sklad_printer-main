...
data class CounterpartyReadRs(
    ...
    val contracts: List<ContractRs>,
    val bankAccounts: List<BankAccountRs>
): RS<CounterpartyAPI>, ReadDTO<Counterparty>
{
    constructor(model:Counterparty):this(
        ...
        model.contracts.map { c -> ContractRs(c) },
        model.bankAccounts.map { a -> BankAccountRs(a) }
    )
}