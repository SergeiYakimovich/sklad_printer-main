<template>
    <div>
        <h1>Заказы</h1>
        <button @click="newClick" class="btn-document-new">Создать заказ...</button>
        <table>
            <tr>
                <th>ID</th>
                <th>Дата</th>
...
export default {
    ...
    created() {
        axios.get(store.getters.apiRoot + 'ordering', {
            headers: {
    ...
    methods: {
        ...
        newClick() {
            this.$router.push('/ordering/new')
        },

        editClick(event) {
            const id = event.target.dataset.item
            this.$router.push('/ordering/' + id)
        },

        deleteClick(event) {
            const id = event.target.dataset.item
            axios.delete(store.getters.apiRoot + 'ordering/' + id, {
                headers: {
                    Authorization: `Bearer ${store.getters.user.token}`
                }
            }).then(resp => {
