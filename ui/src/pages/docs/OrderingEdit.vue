    ...
    <div class="edit-form">
        ...
        <table>
            ...
            <tr v-for="item in document.items" v-bind:key="item.orderNo">
                ...
                <td><input v-model="item.summa" type="number" step="0.01" /></td>
                <td>
                    <select v-model="item.cellId">
...
export default {
    ...
    created() {
        ...
        if (this.uuid != 'new') {
            axios.get(store.getters.apiRoot + 'ordering/' + this.uuid, {
                headers: {
    ...
    methods: {
        ...
        okClick() {
            ...
            if (this.uuid == 'new') {
                axios.post(store.getters.apiRoot + 'ordering', data, {
                    headers: {
                        Authorization: `Bearer ${store.getters.user.token}`
                    }
                }).then(resp => {
                ...
                axios.put(store.getters.apiRoot + 'ordering/' + this.uuid, data, {
                    headers: {
                        Authorization: `Bearer ${store.getters.user.token}`
                    }
                }).then(resp => {
